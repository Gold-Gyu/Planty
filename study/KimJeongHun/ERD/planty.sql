...
CREATE TABLE IF NOT EXISTS `planty`.`gm_info` (
  ...
  `activate` TINYINT(1) NULL DEFAULT '0' COMMENT 'GM 활성화 상태',
  PRIMARY KEY (`gid`))
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `planty`.`user_info` (
  ...
  `emergency_count` INT UNSIGNED NOT NULL DEFAULT '1' COMMENT '사용자 응급실 이용권 개수',
...
CREATE TABLE IF NOT EXISTS `planty`.`plant_info` (
  ...
  `tonic_period` INT NULL DEFAULT NULL COMMENT '식물 영양제 제공 주기',
  `size` VARCHAR(16) NULL DEFAULT NULL COMMENT '크기',
  `place` VARCHAR(16) NULL DEFAULT NULL COMMENT '생육 장소',
  `edible` TINYINT(1) NULL DEFAULT '0' COMMENT '식용 여부',
  PRIMARY KEY (`idx`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `planty`.`subscribe_product` (
  ...
  `period` INT NOT NULL COMMENT '구독 기간',
  `counsulting_cnt` INT UNSIGNED NOT NULL DEFAULT '1' COMMENT '컨설팅 횟수',
...
CREATE TABLE IF NOT EXISTS `planty`.`user_subscribe` (
  ...
  `USER_INFO_uid` INT UNSIGNED NOT NULL COMMENT '사용자 식별키',
  `SUBSCRIBE_PRODUCT_spid` INT UNSIGNED NOT NULL COMMENT '구독상품 식별키',
  `GM_INFO_gid` INT UNSIGNED NOT NULL COMMENT 'GM 식별키',
...
CREATE TABLE IF NOT EXISTS `planty`.`consulting_booking` (
  ...
  `USER_SUBSCRIBE_sid` INT UNSIGNED NOT NULL COMMENT '사용자 구독정보 식별키',
  `USER_INFO_uid` INT UNSIGNED NOT NULL COMMENT '사용자 식별키',
  `GM_INFO_gid` INT UNSIGNED NOT NULL COMMENT 'GM 식별키',
  `TIME_TABLE_idx` INT UNSIGNED NOT NULL COMMENT '시간 식별기',
  `date` DATE NOT NULL DEFAULT (CURRENT_DATE) COMMENT '날짜',
  `cancel` TINYINT NOT NULL DEFAULT '1' COMMENT '취소여부 - true : 취소, false : 취소 안됨',
  `active` TINYINT NOT NULL DEFAULT '0' COMMENT '실행여부',
  PRIMARY KEY (`cid`),
...
CREATE TABLE IF NOT EXISTS `planty`.`consulting_log` (
  `cid` INT UNSIGNED NOT NULL COMMENT '컨설팅 예약 식별기',
...
CREATE TABLE IF NOT EXISTS `planty`.`emergency_log` (
  ...
  `type` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '상담 유형 - 0 : 화상, 1 : 채팅',
  `content` TEXT NULL DEFAULT NULL COMMENT '상담 내용',
  `start_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '실제 시작 시간',
  `end_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '실제 종료 시간',
  PRIMARY KEY (`eid`),
...
CREATE TABLE IF NOT EXISTS `planty`.`product` (
  ...
  `category` INT NOT NULL DEFAULT '0' COMMENT '상품 카테고리 - 0 : 응급실, 1 : 클래스',
  `category_idx` INT NOT NULL COMMENT '상품 카테고리별 식별키 - 응급실 식별키와 클래스 식별키 ',
...
CREATE TABLE IF NOT EXISTS `planty`.`order` (
  ...
  `USER_INFO_uid` INT UNSIGNED NOT NULL COMMENT '사용자 식별키',
  `PRODUCT_idx` INT UNSIGNED NOT NULL COMMENT '상품 식별키',
...
CREATE TABLE IF NOT EXISTS `planty`.`plant_data` (
  `arduino_id` INT UNSIGNED NOT NULL COMMENT '사용자 구독정보 식별키',
  `GM_INFO_gid` INT UNSIGNED NOT NULL COMMENT 'GM 식별키',
  `date` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '일시',
